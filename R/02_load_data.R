# R/02_load_data.R
# ============================================================
# IndepMR — Load data once (merged + wide + SNP instruments)
...
load_data <- function() {
  ...
  if (length(missing_m) > 0) {
    stop(
      paste0(
        "Dataset01.xlsx missing columns: ",
  ...
  if (exists("PATH_SNP_RDS") && !is.null(PATH_SNP_RDS) && file.exists(PATH_SNP_RDS)) {
    
    raw <- readRDS(PATH_SNP_RDS)
