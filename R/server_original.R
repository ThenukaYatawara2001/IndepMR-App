...
server <- function(input, output, session) {
  # ============================================================
  # OpenGWAS JWT token entered by user
  # One token will be used for all OpenGWAS calls
  # ============================================================
  
  observe({
    req(input$opengwas_jwt)
    Sys.setenv(OPENGWAS_JWT = input$opengwas_jwt)
    Sys.setenv(OPENGWAS_JWT_TAB3 = input$opengwas_jwt)
    Sys.setenv(OPENGWAS_JWT_TAB5 = input$opengwas_jwt)
    Sys.setenv(OPENGWAS_JWT_TAB7_MY = input$opengwas_jwt)
    Sys.setenv(OPENGWAS_JWT_TAB7_X = input$opengwas_jwt)
  })
  
  # Debounced confirmation that the token was entered. Debouncing (rather
  # than firing on every keystroke of the passwordInput) means the
  # "added successfully" notification appears once, shortly after the
  # user finishes pasting the token, instead of on every character typed.
  opengwas_jwt_debounced <- shiny::debounce(reactive({ input$opengwas_jwt }), 700)
  
  observeEvent(opengwas_jwt_debounced(), {
    tok <- opengwas_jwt_debounced()
    if (!is.null(tok) && nzchar(trimws(tok))) {
      shiny::showNotification(
        "\u2705 OpenGWAS token added successfully.",
        type = "message",
        duration = 4
      )
    }
  ...
  # Guard used by the 2SMR / MVMR "Run" buttons: blocks the run and pops up
  # a message if no OpenGWAS token has been entered yet.
  require_opengwas_token <- function() {
    tok <- input$opengwas_jwt
    if (is.null(tok) || !nzchar(trimws(tok))) {
      shiny::showModal(shiny::modalDialog(
        title = "OpenGWAS token required",
        "Please paste a valid OpenGWAS JWT token under \u201cOpenGWAS Authentication\u201d (top of the sidebar) before running 2SMR or MVMR analysis.",
        easyClose = TRUE,
  ...
  # ============================================================
  # TAB 2 — Outcome independence vs. Exposure 1
  #
  # LOGIC (simple, main-group level):
  #   Same main group  → same ancestry (Not independent)
  #   Different main group → different ancestry (Independent)
  #   Other / unknown  → NA
  #
  # The second checkbox offers within-ancestry sub-group
  # independence filtering when the exposure has known subgroup
  # resolution (not just a broad continental label).
  # ============================================================

  # Exposure ancestry profile (reactive, used for messages and checkbox)
  exposure_ancestry_info <- reactive({
    ids <- unique(stringr::str_trim(as.character(exposure_mod$selected_gcst())))
    ids <- ids[ids != "" & !is.na(ids)]
    describe_exposure_ancestry(ids, DATA$merged)
  })

  # Level-1 status: same vs. different main ancestry group
  outcome_status_fun <- reactive({
    exp1 <- unique(stringr::str_trim(as.character(exposure_mod$selected_gcst())))
  ...
  # Level-2: within-ancestry sub-group independence
  outcome_subgroup_fun <- reactive({
    ...
    function(out_gcst_vec) {
      vapply(out_gcst_vec, function(oid) {
        get_within_ancestry_status_from_all(oid, exp_gcst, DATA$merged)
      }, character(1))
    }
  })

  # ── Contextual message below the Outcome checkboxes ──────────
  output$outcome_ancestry_msg <- renderUI({
    ids <- unique(stringr::str_trim(as.character(exposure_mod$selected_gcst())))
    ids <- ids[ids != "" & !is.na(ids)]
    if (length(ids) == 0) return(NULL)

    info    <- exposure_ancestry_info()
    same_on <- isTRUE(input$show_only_independent)
    sub_on  <- same_on && isTRUE(input$show_non_overlap_outcome)

    # ── First checkbox OFF ────────────────────────────────────────────────────
    if (!same_on) {
      return(div(class = "ancestry-notice ancestry-warn",
        HTML(paste0(
          "\u26a0\ufe0f  <strong>No ancestry filter is active.</strong> ",
          "Outcomes from all ancestry groups are displayed. ",
          "Using studies from different ancestry populations in 2SMR introduces ",
          "bias from differing LD structure, allele frequencies, and haplotype blocks. ",
          "Enable the same-ancestry filter to ensure valid causal inference."
        ))))
    }

    # ── Exposure has unknown / Other ancestry ─────────────────────────────────
    if (!is.null(info) && info$has_other) {
      return(div(class = "ancestry-notice ancestry-warn",
        HTML(paste0(
          "\u26a0\ufe0f  The selected exposure uses a broad or unknown ancestry label. ",
          "Same-ancestry matching cannot be performed reliably. ",
          "Please consult the GWAS Catalog for the original sample description."
        ))))
    }

    # ── Exposure uses only a broad continental label (e.g. plain "European") ──
    # In this case we cannot determine which specific sub-population the exposure
    # belongs to, so the sub-group independence filter is hidden.
    if (!is.null(info) && info$is_umbrella_only) {
      mg <- if (length(info$main_groups) > 0)
              paste(info$main_groups, collapse = " / ") else "the same group"
      return(div(class = "ancestry-notice ancestry-info",
        HTML(paste0(
          "\u2139\ufe0f  The exposure uses a broad continental ancestry label ",
          "(<strong>", mg, "</strong>) with no specific sub-population identified. ",
          "All outcomes from the <strong>", mg, "</strong> main group are shown. ",
          "The sub-group independence filter is not available: because the exposure's ",
          "specific sub-population is unknown, no outcome sub-group can be guaranteed ",
          "independent from it."
        ))))
    }

    # ── Exposure has specific sub-group resolution ────────────────────────────
    exp_subs <- if (!is.null(info) && length(info$subgroups) > 0)
                  paste(tools::toTitleCase(info$subgroups), collapse = ", ")
                else NULL
    mg <- if (!is.null(info) && length(info$main_groups) > 0)
            paste(info$main_groups, collapse = " / ") else "the same group"

    if (same_on && sub_on) {
      sub_note <- if (!is.null(exp_subs))
        paste0("Outcomes annotated with the broad <strong>", mg, "</strong> label ",
               "are excluded because such studies may include the exposure sub-group ",
               "(<em>", exp_subs, "</em>). ")
      else ""
      return(div(class = "ancestry-notice ancestry-info",
        HTML(paste0(
          "\u2139\ufe0f  Showing outcomes from distinct sub-populations within the ",
          "<strong>", mg, "</strong> main group. ",
          sub_note,
          "Only outcomes with a specific, different sub-group are shown. ",
          "Sample overlap between sub-groups is unlikely; however, LD patterns ",
          "and allele frequencies may still differ. ",
          "Verify that your genetic instruments replicate across these sub-groups."
        ))))
    }

    if (same_on && !sub_on) {
      sub_note <- if (!is.null(exp_subs))
        paste0(" The exposure sub-group is <strong>", exp_subs, "</strong>. ",
               "Note that outcomes using only the broad <em>", mg, "</em> label ",
               "are shown here but cannot be confirmed as sub-group independent.")
      else ""
      return(div(class = "ancestry-notice ancestry-ok",
        HTML(paste0(
          "\u2705  Showing same-ancestry outcomes (<strong>", mg, "</strong>).",
          sub_note, " ",
          "Enable the sub-group filter to restrict to outcomes from a ",
          "distinct, named sub-population."
        ))))
    }
  ...
  # ── Second checkbox: shown only when exposure has specific sub-group resolution ──
  # When the exposure uses only a broad label (e.g. plain "European"), we cannot
  # determine which sub-group it belongs to and therefore cannot guarantee that
  # any outcome sub-group is independent from it.  The checkbox is hidden.
  output$outcome_subgroup_checkbox_ui <- renderUI({
    if (!isTRUE(input$show_only_independent)) return(NULL)
    info <- exposure_ancestry_info()
    if (!is.null(info) && (info$is_umbrella_only || info$has_other)) return(NULL)
    checkboxInput(
      "show_non_overlap_outcome",
      label = "Within same ancestry, show only sub-group independent Outcome studies",
      value = FALSE
    )
  })

  # Outcome selection module
  outcome_mod <- searchServer(
    ...
    subgroup_indep_fun_reactive      = outcome_subgroup_fun,
  ...
  observeEvent(input$run_mr, {
    if (!require_opengwas_token()) return()
  ...
  # Warn when the Steiger directionality test is being applied to a binary
  # outcome (see is_binary_trait() / steiger_binary_notice() in
  # 06_mr_helpers.R for the underlying rationale).
  output$steiger_binary_msg <- renderUI({
    r <- current_pair_res()
    if (is.null(r) || !is.null(r$error)) return(NULL)
    steiger_binary_notice(is_binary_trait(r$outcome_gcst, DATA$merged), label = "Outcome")
  })
  ...
  # --------------------------------------------------------------------
  # TAB 4: REPEATED SNP
  # Manage repeated SNP table (ancestry-independence filtering only —
  # the old Top/Sub Category filter, which depended on
  # paths_AND_SUBGROUP.xlsx, has been removed).
  # --------------------------------------------------------------------
  ...
  repeated_tbl_raw <- reactive({
    ...
    if (is.null(DATA$snp) || nrow(DATA$snp) == 0 || !all(c("GCST", "SNP") %in% names(DATA$snp))) {
      return(data.frame(Message = "Dataset02.rds missing or must contain GCST and SNP columns."))
    }
    ...
    # Independence (first checkbox) is evaluated using only Exposure 1 as
    # reference — this matches Tab 4-B's tab7_ref_ids(), which also uses
    # only the first selected Exposure 1 GCST for its "same-ancestry"
    # checkbox.
    ...
    # ── Within-ancestry sub-group independence column ────────────────────────
    # Computed once and stored as both a display label and raw code.
    # The raw code is used for the second checkbox filter.
    #
    # Reference set matches Tab 4-B's mediator_subgroup_fun(): a candidate
    # must be sub-group independent from BOTH Exposure 1 AND the Outcome,
    # since a study picked here becomes the Exposure 2 / Mediator, and MVMR
    # requires ancestry independence across all three (Exposure 1, Outcome,
    # and Mediator).
    subgroup_ref_ids <- unique(c(r$exposure_gcst, r$outcome_gcst))
    subgroup_ref_ids <- subgroup_ref_ids[subgroup_ref_ids != "" & !is.na(subgroup_ref_ids)]
    cand$`_subgroup_raw` <- vapply(cand$GCST, function(cid) {
      get_within_ancestry_status_from_all(cid, subgroup_ref_ids, DATA$merged)
    }, character(1))
    cand$`Sub-group` <- dplyr::case_when(
      cand$`_subgroup_raw` == "SUBGROUP_INDEPENDENT" ~ "Independent",
      cand$`_subgroup_raw` == "SUBGROUP_OVERLAPPING"  ~ "Overlapping",
      cand$`_subgroup_raw` == "UMBRELLA_ONLY"         ~ "Broad only",
      cand$`_subgroup_raw` == "OTHER_PRESENT"         ~ "Broad / unknown",
      TRUE                                             ~ "\u2014"
    )
    
    # ── Apply filters based on checkbox state ──────────────────────────────────
    # First checkbox (rep_only_indep): retain only same-ancestry candidates.
    # Second checkbox (rep_non_overlap): further restrict to candidates whose
    # ancestry profile is subgroup-independent from the exposure reference set.
    if (isTRUE(input$rep_only_indep)) {
      ...
      if (!is.null(input$rep_non_overlap) && isTRUE(input$rep_non_overlap)) {
        cand <- cand %>% dplyr::filter(`_subgroup_raw` == "SUBGROUP_INDEPENDENT")
      }
    }
    ...
    # Final table — clean columns, no COHORT, Ancestry + Sub-group shown
    cand %>%
      dplyr::select(-dplyr::any_of("_subgroup_raw")) %>%
      transmute(
        GCST,
        MAPPED_TRAIT,
        overlap_snps,
  ...
  output$tbl_repeated <- renderDT({
    ...
    # Recode raw independence value to readable ancestry status
    df$`Ancestry Status` <- dplyr::recode(
      df$`Ancestry Status`,
      "Independent"    = "Different",
      "Not independent"= "Same"
    )
    df <- df[, !names(df) %in% "_subgroup_raw", drop = FALSE]
    df <- df %>% dplyr::relocate(`Data Type`, .after = `Trait Name`)
    ...
    datatable(df,
      ...
      formatStyle(
        ...
        color = styleEqual(
          ...
          c("#fb7185", "#34d399", "#fbbf24")
        )
      ) %>%
      formatStyle(
        "Sub-group",
        color = styleEqual(
          c("Independent", "Overlapping", "Broad only", "Broad / unknown", "\u2014"),
          c("#34d399", "#fb7185", "#fbbf24", "#f97316", "#94a3b8")
        )
      )
  })

  # ── Repeated-SNP ancestry context message ──────────────────────────────────
  output$rep_ancestry_msg <- renderUI({
    same_on <- isTRUE(input$rep_only_indep)
    sub_on  <- same_on && isTRUE(input$rep_non_overlap)
    if (!same_on) return(NULL)

    df <- repeated_tbl_raw()
    if ("Message" %in% names(df) || nrow(df) == 0) return(NULL)

    pair_res <- current_pair_res()
    exp_ids  <- if (!is.null(pair_res)) unique(pair_res$exposure_gcst) else character(0)
    exp_ids  <- exp_ids[exp_ids != "" & !is.na(exp_ids)]
    info     <- describe_exposure_ancestry(exp_ids, DATA$merged)

    mg <- if (!is.null(info) && length(info$main_groups) > 0)
            paste(info$main_groups, collapse = " / ") else "the same group"

    # Exposure is umbrella-only: cannot resolve sub-group independence
    if (!is.null(info) && (info$has_other || info$is_umbrella_only)) {
      return(div(class = "ancestry-notice ancestry-info",
        HTML(paste0(
          "\u2139\ufe0f  The exposure uses a broad ancestry label ",
          "(<strong>", mg, "</strong>) with no specific sub-population identified. ",
          "Sub-group independence cannot be assessed, so the sub-group filter is hidden. ",
          "Candidates from the same main ancestry group are shown."
        ))))
    }
    ...
    if (sub_on) {
      sub_note <- if (!is.null(exp_subs))
        paste0("Candidates annotated with only the broad <em>", mg,
               "</em> label are excluded because such studies may include the ",
               "exposure sub-group (<em>", exp_subs, "</em>). ")
      else ""
      return(div(class = "ancestry-notice ancestry-info",
        HTML(paste0(
          "\u2139\ufe0f  Showing candidates from distinct sub-populations within the ",
          "<strong>", mg, "</strong> main group. ",
          sub_note,
          "LD patterns may still differ between sub-groups. ",
          "Verify instrument replication before using as Exposure 2 / Mediator."
        ))))
    }

    sub_note <- if (!is.null(exp_subs))
      paste0(" Exposure sub-group: <strong>", exp_subs, "</strong>. ",
             "Note that candidates using only the broad <em>", mg,
             "</em> label are included but shown as <em>Broad only</em> in the ",
             "Sub-group column; they cannot be confirmed as sub-group independent.")
    else ""
    div(class = "ancestry-notice ancestry-ok",
      HTML(paste0(
        "\u2705  Showing candidates from the same main ancestry group ",
        "(<strong>", mg, "</strong>).",
        sub_note, " ",
        "Enable the sub-group filter to restrict to candidates from a ",
        "distinct, named sub-population."
      )))
  })
  ...
  # --------------------------------------------------------------------
  # Ancestry resolution flags
  #
  # has_subgroup_resolution_exp:
  #   TRUE when the selected exposure has specific subgroup/member-level
  #   ancestry resolution (i.e., not just a broad continental label such
  #   as "European").  Controls visibility of the second checkbox.
  #
  # has_subgroup_resolution_union:
  #   TRUE when both exposure AND outcome have subgroup resolution.
  #
  # These are exposed as output$ values so that conditionalPanel()
  # in the UI can reference them.
  # --------------------------------------------------------------------

  has_subgroup_resolution_exp <- reactive({
    ids  <- unique(stringr::str_trim(as.character(exposure_mod$selected_gcst())))
    ids  <- ids[ids != "" & !is.na(ids)]
    if (length(ids) == 0) return(FALSE)
    info <- describe_exposure_ancestry(ids, DATA$merged)
    if (is.null(info)) return(FALSE)
    !info$has_other && !info$is_umbrella_only
  })

  has_subgroup_resolution_union <- reactive({
    if (!has_subgroup_resolution_exp()) return(FALSE)
    out_ids <- unique(stringr::str_trim(as.character(outcome_mod$selected_gcst())))
    out_ids <- out_ids[out_ids != "" & !is.na(out_ids)]
    if (length(out_ids) == 0) return(FALSE)
    info_out <- describe_exposure_ancestry(out_ids, DATA$merged)
    if (is.null(info_out)) return(FALSE)
    !info_out$has_other && !info_out$is_umbrella_only
  })

  # Expose as output$ — names kept for UI conditionalPanel compatibility
  output$has_cohort_exp   <- renderText({ has_subgroup_resolution_exp() })
  output$has_cohort_union <- renderText({ has_subgroup_resolution_union() })
  output$has_cohort_pair  <- renderText({ has_subgroup_resolution_exp() })
  ...
  # --------------------------------------------------------------------
  # Contextual ancestry warnings
  #
  # Fires a Shiny notification when a selected study has broad or
  # unknown ancestry ("Other"), since ancestry-based independence
  # cannot be reliably determined in that case.
  # --------------------------------------------------------------------
  warn_for_gcst <- function(ids, role) {
    ...
    has_other <- any(vapply(ids, function(gid) {
      prof <- get_profile_for_gcst(gid, DATA$merged)
      if (is.null(prof)) return(FALSE)
      prof$has_other
    }, logical(1)))
    ...
    if (has_other) {
      shiny::showNotification(
        paste0(
          "\u26a0\ufe0f  Selected ", role, " uses a broad or unknown ancestry label. ",
          "Ancestry-based independence cannot be determined reliably. ",
          "Please check the original study in the GWAS Catalog."
        ),
        type = "warning",
        duration = 10
      )
  ...
  # Warn when the Steiger directionality test (Exposure 1 -> Mediator) is
  # being applied to a binary mediator.
  output$steiger_binary_msg_tab5 <- renderUI({
    r <- tab5_mr_rv$res
    if (is.null(r)) return(NULL)
    steiger_binary_notice(is_binary_trait(r$outcome_gcst, DATA$merged), label = "Mediator")
  })
  ...
  # ============================================================
  # TAB 7 — Mediator search module
  #
  # Independence requirements for mediators are stricter than for
  # outcomes: the mediator must be ancestry-independent from BOTH
  # the exposure (Exposure 1) AND the outcome, since MVMR uses
  # instruments for two exposures simultaneously.
  #
  # The second checkbox ("sub-group independent") uses the same
  # ancestry-hierarchy logic as the outcome tab — no cohort data.
  # ============================================================

  tab7_only_non_overlap <- reactive({ isTRUE(input$tab7_non_overlap) })

  # Within-ancestry sub-group independence function for mediators.
  # Compares candidates against BOTH Exposure 1 and Outcome reference sets.
  mediator_subgroup_fun <- reactive({
    ...
    ref     <- unique(c(exp_ref, out_ref))
    ref     <- ref[ref != "" & !is.na(ref)]
    if (length(ref) == 0) return(NULL)
    function(gcsts) {
      vapply(gcsts, function(gid) {
        get_within_ancestry_status_from_all(gid, ref, DATA$merged)
      }, character(1))
    }
  })

  # Mediator ancestry context message
  output$tab7_ancestry_msg <- renderUI({
    same_on <- isTRUE(input$tab7_only_indep)
    sub_on  <- same_on && isTRUE(input$tab7_non_overlap)

    exp_ids <- unique(stringr::str_trim(as.character(exposure_mod$selected_gcst())))
    exp_ids <- exp_ids[exp_ids != "" & !is.na(exp_ids)]
    if (length(exp_ids) == 0) return(NULL)
    ...
    if (!same_on) {
      return(div(class = "ancestry-notice ancestry-warn",
        HTML(paste0(
          "\u26a0\ufe0f  <strong>No ancestry filter is active for the Mediator.</strong> ",
          "Mediators from any ancestry group are shown. ",
          "MVMR requires matched ancestry across all datasets for valid estimation ",
          "of direct causal effects."
        ))))
    }

    # ── Exposure has unknown ancestry ─────────────────────────────────────────
    if (!is.null(info) && info$has_other) {
      return(div(class = "ancestry-notice ancestry-warn",
        HTML(paste0(
          "\u26a0\ufe0f  The exposure has a broad or unknown ancestry label. ",
          "Ancestry-based mediator filtering cannot be applied reliably. ",
          "Please verify the sample description in the GWAS Catalog."
        ))))
    }

    # ── Exposure uses only a broad continental label ──────────────────────────
    if (!is.null(info) && info$is_umbrella_only) {
      return(div(class = "ancestry-notice ancestry-info",
        HTML(paste0(
          "\u2139\ufe0f  The exposure uses a broad continental ancestry label ",
          "(<strong>", mg, "</strong>) with no specific sub-population identified. ",
          "Mediators from the same main group are shown. ",
          "The sub-group independence filter is not available because the exposure's ",
          "specific sub-population is unknown."
        ))))
    }
    ...
    if (sub_on) {
      sub_note <- if (!is.null(exp_subs))
        paste0("Mediator candidates annotated with only the broad <em>", mg,
               "</em> label are excluded, as they may include the ",
               "exposure sub-group (<em>", exp_subs, "</em>). ")
      else ""
      return(div(class = "ancestry-notice ancestry-info",
        HTML(paste0(
          "\u2139\ufe0f  Showing mediators from distinct sub-populations within the ",
          "<strong>", mg, "</strong> main group. ",
          sub_note,
          "LD patterns may still differ between sub-groups. ",
          "Verify instrument replication across sub-populations before running MVMR."
        ))))
    }

    sub_note <- if (!is.null(exp_subs))
      paste0(" Exposure sub-group: <strong>", exp_subs, "</strong>. ",
             "Mediators using only the broad <em>", mg,
             "</em> label are included but shown as <em>Broad only</em>; ",
             "they cannot be confirmed as sub-group independent.")
    else ""
    div(class = "ancestry-notice ancestry-ok",
      HTML(paste0(
        "\u2705  Showing mediators with the same main ancestry group as the exposure ",
        "(<strong>", mg, "</strong>).",
        sub_note, " ",
        "Enable the sub-group filter to restrict to mediators from a distinct ",
        "named sub-population."
      )))
  })

  # Conditional second checkbox for mediator tab
  output$tab7_subgroup_checkbox_ui <- renderUI({
    if (!isTRUE(input$tab7_only_indep)) return(NULL)
    exp_ids <- unique(stringr::str_trim(as.character(exposure_mod$selected_gcst())))
    exp_ids <- exp_ids[exp_ids != "" & !is.na(exp_ids)]
    info <- describe_exposure_ancestry(exp_ids, DATA$merged)
    if (!is.null(info) && (info$is_umbrella_only || info$has_other)) return(NULL)
    checkboxInput(
      "tab7_non_overlap",
      label = "Within same ancestry, show only sub-group independent Mediator studies",
      value = FALSE
    )
  })
  ...
  tab7_search <- searchServer(
    ...
    subgroup_indep_fun_reactive      = mediator_subgroup_fun,
    filter_only_non_overlap_reactive = tab7_only_non_overlap,
    enable_selection                 = TRUE,
    restrict_accessions_reactive = reactive({
      if (!isTRUE(input$tab7_only_indep)) return(character(0))
      ref <- tab7_ref_ids()
      if (length(ref) == 0) return(character(0))
            ...
            column(
              ...
              div(
                ...
                conditionalPanel(
                  ...
                  DT::DTOutput("tbl_mediation_mvmr"),
                  ...
                  DT::DTOutput("tbl_mediation_2smr"),
