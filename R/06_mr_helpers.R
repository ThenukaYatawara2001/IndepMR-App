...
# ============================================================
# Binary-trait detection + Steiger binary-outcome warning
# ============================================================
#
# TwoSampleMR::directionality_test() / mr_steiger() estimate the variance
# explained (R^2) by the instruments in the exposure and in the outcome.
# When no pre-calculated r.exposure / r.outcome is supplied, the function
# assumes BOTH traits are quantitative and warns:
#   "r.exposure and/or r.outcome not present. Calculating approximate
#    SNP-exposure and/or SNP-outcome correlations, assuming all are
#    quantitative traits. Please pre-calculate r.exposure and/or r.outcome
#    using get_r_from_lor() for any binary traits"
# This app does not pre-supply case/control-adjusted correlations, so for
# a binary outcome (or, in the X -> Mediator pathway, a binary mediator)
# the Steiger R^2 / p-value is only an approximation on the log-odds scale
# treated as if continuous, and the inferred causal direction can be
# unreliable. We surface that limitation to the user instead of silently
# presenting the table as if it were valid for any trait type.

# Returns TRUE / FALSE / NA for whether a GCST study is a binary trait,
# based on the "Data Type" column in the merged metadata table.
is_binary_trait <- function(gcst_id, merged_df) {
  gcst_id <- stringr::str_trim(as.character(gcst_id))
  if (length(gcst_id) == 0 || is.na(gcst_id) || gcst_id == "") return(NA)
  if (is.null(merged_df) || !("Data Type" %in% names(merged_df))) return(NA)
  row <- merged_df %>% dplyr::filter(`STUDY ACCESSION` == gcst_id)
  if (nrow(row) == 0) return(NA)
  identical(as.character(row$`Data Type`[1]), "Binary")
}

# Ancestry-notice-styled warning box shown above the Steiger / Causal
# direction test table whenever the relevant trait (Outcome, or Mediator
# for the Exposure 1 -> Mediator pathway) is binary.
steiger_binary_notice <- function(is_binary, label = "Outcome") {
  if (!isTRUE(is_binary)) return(NULL)
  shiny::div(
    class = "ancestry-notice ancestry-warn",
    shiny::HTML(paste0(
      "\u26a0\ufe0f  <strong>", label, " is binary \u2014 the Causal direction test ",
      "(Steiger) may be inappropriate here.</strong> Steiger directionality assumes ",
      "a quantitative trait when estimating variance explained (R\u00b2) unless ",
      "case/control-adjusted SNP\u2013", tolower(label), " correlations are supplied ",
      "(via <code>get_r_from_lor()</code>), which this app does not currently do. ",
      "For a binary ", tolower(label), ", treat the R\u00b2 and p-value below as an ",
      "approximation only, and interpret the inferred causal direction with caution."
    ))
  )
}
